...
- 文件上传：先保存为管理员可见的草稿，在资料库点击“审核发布”后后台建立索引。
...
- 管理数据中心提供运行指标、操作审计和后台任务。新版评测调用完整问答链路，检查期望来源、必含词和禁用词，会使用当前配置的模型额度。
...
仓库包含完整可交接源码，但最新草稿发布、回收站、后台任务和端到端评测功能仍属于**待验收版本**，不要直接覆盖生产环境。此前在服务器临时目录运行了 47 项自动测试并全部通过；尚未完成网页全流程验收和正式部署。IT 应先在隔离环境测试上传、发布失败回滚、权限变化、回收与恢复、并发同步以及服务重启。

后台任务当前使用单进程线程池和 SQLite 任务记录，不是 Redis 分布式队列；重启中的任务不会自动续跑。回收站目前覆盖服务器本地文件，删除在线来源是删除连接，不能恢复远端文件。云端文件副本的删除和发布失败清理也需要单独验收。上传草稿不替代独立审核员或多级审批制度。

部署与数据交接说明见 [IT 交接清单](docs/it-handoff.md)。本仓库不包含生产数据或真实凭证。
